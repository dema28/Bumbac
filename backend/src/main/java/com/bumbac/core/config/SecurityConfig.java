...
public class SecurityConfig {
  ...
  public CacheManager cacheManager() {
    ...
    cacheManager.setCacheNames(java.util.Arrays.asList(
        ...
        "roles", // Кэш ролей
        "carts", // Кэш корзин по userId
        "cartItems", // Кэш товаров в корзине
        "colors", // Кэш цветов товаров
        "yarns", // Кэш пряжи
        "orders", // Кэш заказов
        "payments", // Кэш платежей
        "returns", // Кэш возвратов
        "shipments", // Кэш доставки
        "favorites" // Кэш избранного пользователей
    ));
    return cacheManager;
  }
  ...
  public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
    http
        ...
        .authorizeHttpRequests(auth -> auth
            ...
            // Публичные GET каталога (контроллеры с /api, контекст-путь тоже /api →
            // внутренний путь /api/…)
        ...
        // Регистрируем наш DaoAuthenticationProvider (иначе может взяться дефолтный без
        // BCrypt)
